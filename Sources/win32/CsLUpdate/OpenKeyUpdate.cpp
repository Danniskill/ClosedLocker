...
DWORD WINAPI UpdateThreadFunction(LPVOID lpParam) {
	...
	if (res == S_OK) {
		...
		MessageBox(hDlg, _T("Có lỗi trong quá trình cập nhật, vui lòng thử lại sau!"), _T("CsL Update"), MB_OK);
		ExitProcess(0);
		return 0;
	}
	...
	wsprintf(updateUrl, TEXT("https://github.com/tuyenvm/OpenKey/releases/download/%s/CsL%s-Windows.zip"),
		versionName.c_str(),
		versionName.c_str());
	wsprintf(path, TEXT("%s\\_CsLUpdate.zip"), currentDir);
	res = URLDownloadToFile(NULL, updateUrl, path, 0, NULL);

	if (res == S_OK) {
		//remove old file
		DeleteFile(L"CsL.exe");
		//extract zip file
		WinExec("powershell.exe -NoP -NonI -Command \"Expand-Archive '.\\_CsLUpdate.zip' '.\\_CsLUpdate'\" ", SW_HIDE);
		Sleep(5000);
		MoveFile(L"_CsLUpdate\\CsL.exe", L"CsL.exe");
		DeleteFile(path);
		DeleteFile(L"_CsLUpdate\\CsLUpdate.exe");
		DeleteFile(L"_CsLUpdate\\CsL.exe");
		DeleteFile(L"_CsLUpdate\\CsL32.exe");
		RemoveDirectory(L".\\_CsLUpdate");
		MessageBox(hDlg, _T("Bạn đã cập nhật CsL bản mới nhất thành công!"), _T("CsL Update"), MB_OK);
		ExitProcess(0);
	} else {
		MessageBox(hDlg, _T("Có lỗi trong quá trình cập nhật, vui lòng thử lại sau!"), _T("CsL Update"), MB_OK);
		ExitProcess(0);
