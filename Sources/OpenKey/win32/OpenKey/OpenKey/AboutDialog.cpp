...
void AboutDialog::onUpdateButton() {
	...
	if (OpenKeyManager::checkUpdate(newVersion)) {
		WCHAR msg[256];
		wsprintf(msg, 
			TEXT("ClosedLocker Có phiên bản mới (%s), bạn có muốn cập nhật không?"),
			utf8ToWideString(newVersion).c_str());

		int msgboxID = MessageBox(
			hDlg,
			msg,
			_T("ClosedLocker Update"),
		...
		if (msgboxID == IDYES) {
			...
			wsprintf(path, TEXT("%s\\ClosedLockerUpdate.exe"), path);
		...
		MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("ClosedLocker Update"), MB_OK);
	}
