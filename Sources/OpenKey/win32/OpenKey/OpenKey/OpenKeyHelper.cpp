...
static LPCTSTR sk = TEXT("SOFTWARE\\Chunk\\ClosedLocker");
static HKEY hKey;
...
static HANDLE _proc;
static string _exeNameUtf8 = "TheClosedLockerProject";
...
int CF_OPENKEY = RegisterClipboardFormat(_T("ClosedLocker Format"));
...
void OpenKeyHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			...
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn ClosedLocker /rl highest /tr \"%s\" /f", path.c_str());
			WinExec(buff, SW_HIDE);
		} else {
			RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
			wstring path = getFullPath();
			RegSetValueEx(hKey, _T("ClosedLocker"), 0, REG_SZ, (byte*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	}
	else {
		RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("ClosedLocker"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn ClosedLocker /f", SW_HIDE);
	}
}
...
string& OpenKeyHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("ClosedLocker.exe")) == 0 ||
		wcscmp(_exeName, _T("ClosedLocker32.exe")) == 0 || 
...
wstring OpenKeyHelper::getContentOfUrl(LPCTSTR url){
	...
	wsprintf(path, TEXT("%s\\_ClosedLocker.tempf"), path);
