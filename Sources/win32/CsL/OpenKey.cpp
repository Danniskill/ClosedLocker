...
void OpenKeyInit() {
	...
	// Force removed features to remain disabled (0)
	vCheckSpelling = 0;
...
LRESULT CALLBACK keyboardHookProcess(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	//ignore my event or injected events
	if (keyboardData->dwExtraInfo != 0 || (keyboardData->flags & LLKHF_INJECTED)) {
		return CallNextHookEx(hKeyboardHook, nCode, wParam, lParam);
	}

	// Sync modifier keys state for non-modifier keys to prevent stuck Shift/Ctrl/Alt keys
	bool isMod = (keyboardData->vkCode == VK_LSHIFT || keyboardData->vkCode == VK_RSHIFT ||
	              keyboardData->vkCode == VK_LCONTROL || keyboardData->vkCode == VK_RCONTROL ||
	              keyboardData->vkCode == VK_LMENU || keyboardData->vkCode == VK_RMENU ||
	              keyboardData->vkCode == VK_LWIN || keyboardData->vkCode == VK_RWIN ||
	              keyboardData->vkCode == VK_NUMLOCK || keyboardData->vkCode == VK_CAPITAL ||
	              keyboardData->vkCode == VK_SCROLL);

	if (!isMod) {
		_flag = 0;
		if (GetKeyState(VK_SHIFT) < 0) _flag |= MASK_SHIFT;
		if (GetKeyState(VK_CONTROL) < 0) _flag |= MASK_CONTROL;
		if (GetKeyState(VK_MENU) < 0) _flag |= MASK_ALT;
		if (GetKeyState(VK_LWIN) < 0 || GetKeyState(VK_RWIN) < 0) _flag |= MASK_WIN;
...
VOID CALLBACK winEventProcCallback(HWINEVENTHOOK hWinEventHook, DWORD dwEvent, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
	// Reinstall hooks to prevent them from being silently removed by Windows or getting lost
	UnhookWindowsHookEx(hKeyboardHook);
	UnhookWindowsHookEx(hMouseHook);
	HINSTANCE hInstance = GetModuleHandle(NULL);
	hKeyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, keyboardHookProcess, hInstance, 0);
