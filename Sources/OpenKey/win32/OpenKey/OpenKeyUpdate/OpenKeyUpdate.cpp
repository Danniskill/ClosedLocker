...
DWORD WINAPI UpdateThreadFunction(LPVOID lpParam) {
	...
	wsprintf(updateUrl, TEXT("https://github.com/tuyenvm/OpenKey/releases/download/%s/ClosedLocker%s-Windows.zip"),
		versionName.c_str(),
		versionName.c_str());
	wsprintf(path, TEXT("%s\\_ClosedLockerUpdate.zip"), currentDir);
	res = URLDownloadToFile(NULL, updateUrl, path, 0, NULL);

	if (res == S_OK) {
		//remove old file
		DeleteFile(L"ClosedLocker.exe");
		//extract zip file
		WinExec("powershell.exe -NoP -NonI -Command \"Expand-Archive '.\\_ClosedLockerUpdate.zip' '.\\_ClosedLockerUpdate'\" ", SW_HIDE);
		Sleep(5000);
		MoveFile(L"_ClosedLockerUpdate\\ClosedLocker.exe", L"ClosedLocker.exe");
		DeleteFile(path);
		DeleteFile(L"_ClosedLockerUpdate\\ClosedLockerUpdate.exe");
		DeleteFile(L"_ClosedLockerUpdate\\ClosedLocker.exe");
		DeleteFile(L"_ClosedLockerUpdate\\ClosedLocker32.exe");
		RemoveDirectory(L".\\_ClosedLockerUpdate");
		MessageBox(hDlg, _T("Bạn đã cập nhật ClosedLocker bản mới nhất thành công!"), _T("ClosedLocker Update"), MB_OK);
		ExitProcess(0);
	} else {
		MessageBox(hDlg, _T("Có lỗi trong quá trình cập nhật, vui lòng thử lại sau!"), _T("ClosedLocker Update"), MB_OK);
		ExitProcess(0);
	}
	return 0;
}
